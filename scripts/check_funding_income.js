const https = require('https');
const crypto = require('crypto');
const fs = require('fs');

const env = {};
fs.readFileSync('/home/bharatos_user/bharatos/.env', 'utf8').split('\n').forEach(l => {
  const i = l.indexOf('=');
  if (i > 0) env[l.slice(0, i).trim()] = l.slice(i + 1).trim();
});

const ts = Date.now();
const qs = 'incomeType=FUNDING_FEE&limit=10&timestamp=' + ts + '&recvWindow=10000';
const sig = crypto.createHmac('sha256', env.BINANCE_SECRET_KEY).update(qs).digest('hex');
const url = 'https://fapi.binance.com/fapi/v1/income?' + qs + '&signature=' + sig;

https.get(url, { headers: { 'X-MBX-APIKEY': env.BINANCE_API_KEY } }, r => {
  let d = '';
  r.on('data', c => d += c);
  r.on('end', () => {
    console.log('=== 💰 RECENT FUNDING FEE PAYMENTS ===');
    const items = JSON.parse(d);
    if (Array.isArray(items)) {
      items.forEach(it => {
        const date = new Date(it.time).toLocaleString('en-IN', { timeZone: 'Asia/Kolkata' });
        console.log(`[${date}] ${it.symbol} -> Income: ${it.income} USDT (${it.asset})`);
      });
      if (items.length === 0) console.log('No payments found yet.');
    } else {
      console.log('Response:', items);
    }
  });
}).end();
